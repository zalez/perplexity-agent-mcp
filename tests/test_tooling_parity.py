...
# Bounded forward scan for `rev:` after a `repo:` anchor line (Finding 4).
...
def _pin_from_precommit_text(text: str, tool: str) -> str | None:
    ...
    Anchored to a `repo:` line naming `tool` (Finding 2): `re.match` only
    succeeds if the match starts at the line's first non-whitespace
    character, and a real YAML comment always starts with `#` there -- so a
    comment merely mentioning `tool` can never open with `repo:` and can
    never satisfy this. No separate comment filter is needed.
    ...
    (Finding 4): pre-commit's per-repo mapping keys (`repo`, `rev`, `hooks`)
    are unordered YAML, so `hooks:` -- and everything nested under it -- may
    ...
    optional `v` prefix (Finding 3), not a fixed `X.Y.Z` shape -- so a
    pre-release suffix like `0.16.0rc1` survives intact instead of being
    truncated to `0.16.0` and made to look like a match for the final
    release.
    """
...
def _pin_from_ci_text(text: str, tool: str) -> str | None:
    ...
    Anchored (Finding 2) to a real `run: pip install ...` line via
    `re.MULTILINE` + `^`: a lookalike mention elsewhere in the file, such as
    a comment reading "# TODO: bump to ruff==0.16.0", has no `run:` at its
    own line's start and so cannot satisfy the match, regardless of where it
    sits relative to the real pin.

    The captured token is the full non-whitespace run after `tool==`
    (Finding 3), preserving a pre-release/build suffix intact instead of
    truncating it to a bare `X.Y.Z`.
    """
...
class TestToolingParity(unittest.TestCase):
    def _assert_versions_match(self, precommit_tool: str, ci_tool: str) -> None:
        """Finding 1: each parity test must be self-sufficient. Both sides
        are asserted non-None *here*, inside the same test that compares
        them, so a pin silently deleted from BOTH files can never produce a
        vacuous `None == None` pass -- this fails on one of the two
        assertIsNotNone calls below every time, independent of whatever
        else this file does or doesn't also check.
        """
...
class TestPinExtractionIgnoresLookalikeComments(unittest.TestCase):
    """Finding 2: `re.search` takes the leftmost match, so an unanchored
    pattern would prefer a comment mentioning the tool over the real pin
    sitting below it. These prove the structural anchors in
    `_pin_from_ci_text` / `_pin_from_precommit_text` aren't fooled by that.
    """
...
class TestPinCapturePreservesPrereleaseSuffixes(unittest.TestCase):
    """Finding 3: capturing a fixed `[0-9]+\\.[0-9]+\\.[0-9]+` shape silently
    drops a 4th component or a pre-release suffix, so "0.16.0rc1" would be
...
class TestPrecommitPinSearchToleratesKeyReordering(unittest.TestCase):
    """Finding 4: pre-commit's per-repo YAML mapping keys are unordered, so
    requiring `rev:` on the line immediately after `repo:` is a positional
    assumption that a valid reordering can defeat.
    """
